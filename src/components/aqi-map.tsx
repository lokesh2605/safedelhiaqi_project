...
import { predictAQI } from "@/lib/predict-api";
...
function MapClickHandler({ stations, onAreaClick }: MapClickHandlerProps) {
  useMapEvents({
    async click(e) {
      const result = findNearestStationAndAQI(
        e.latlng.lat,
        e.latlng.lng,
        stations
      );
      ...
      try {
        ...
        const prediction = await predictAQI({
          lat: e.latlng.lat,
          lng: e.latlng.lng,
          station_aqi: result.station.aqi,
        ...
        onAreaClick({
          ...
          distance: result.distance,
          estimatedAQI: prediction.predicted_aqi ?? result.estimatedAQI,
        });
